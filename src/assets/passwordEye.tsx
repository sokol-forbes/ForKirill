...
interface IPasswordEyeIconProps {
  type: keyof typeof eye;
  size: number;
  className?: string;
  onClick?: (event: MouseEvent<HTMLOrSVGElement>) => void;
}

export const PasswordEyeIcon = ({ type, size = 20, className, onClick }: IPasswordEyeIconProps) => {
  return (
    <svg onClick={onClick} width={size} height={size} className={className} fill="none" viewBox="0 0 24 24">
      {eye[type]}
    </svg>
  );
};
